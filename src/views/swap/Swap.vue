...
import { getcheRouterAddress } from '@/utils/addressHelp';
import { MaxUint256 } from '@ethersproject/constants';
import { getErc20Contract, getcheRouterContract, getchePairContract, getcheFactoryContract } from '@/utils/contractHelp';
import SettingsModal from '@/components/SettingsModal.vue';
import { simpleRpcProvider } from '@/utils/provider';
import { usecheRouterContract, useErc20Contract, useWethContract } from '@/utils/useContract';
  ...
  methods: {
    ...
    handleConfirm() {
      ...
      if (isBnb(this.fromCur) && this.inputSource == 'from') {
        this.swapExactQKIForTokens(amountIn, amountOutMin, path, to, deadline);
        return;
      }

      if (isBnb(this.fromCur) && this.inputSource == 'to') {
        this.swapQKIForExactTokens(amountOut, amountInMax, path, to, deadline);
        return;
      }

      if (isBnb(this.toCur) && this.inputSource == 'from') {
        this.swapExactTokensForQKI(amountIn, amountOutMin, path, to, deadline);
        return;
      }

      if (isBnb(this.toCur) && this.inputSource == 'to') {
        this.swapTokensForExactQKI(amountOut, amountInMax, path, to, deadline);
        return;
      }
    ...
    async swapExactQKIForTokens(amountIn, amountOutMin, path, to, deadline) {
      const cheRouterContract = usecheRouterContract();
      const gas = await cheRouterContract.methods.swapExactQKIForTokens(amountOutMin, path, to, deadline).estimateGas({ from: this.account, value: amountIn });
      this.swapDetailShow = false;
      this.pendingDetailShow = true;
      cheRouterContract.methods
        .swapExactQKIForTokens(amountOutMin, path, to, deadline)
    ...
    async swapQKIForExactTokens(amountOut, amountInMax, path, to, deadline) {
      const cheRouterContract = usecheRouterContract();
      const gas = await cheRouterContract.methods
        .swapQKIForExactTokens(amountOut, path, to, deadline)
        .estimateGas({ from: this.account, value: amountInMax })
        .catch(async e => {
          const gas = await cheRouterContract.methods.swapExactQKIForTokensSupportingFeeOnTransferTokens(amountOut, path, to, deadline).estimateGas({ from: this.account, value: amountInMax });
          this.swapDetailShow = false;
          this.pendingDetailShow = true;
          cheRouterContract.methods
            .swapExactQKIForTokensSupportingFeeOnTransferTokens(amountOut, path, to, deadline)
    ...
    async swapExactTokensForQKI(amountIn, amountOutMin, path, to, deadline) {
      const cheRouterContract = usecheRouterContract();
      const gas = await cheRouterContract.methods.swapExactTokensForQKI(amountIn, amountOutMin, path, to, deadline).estimateGas({ from: this.account });
      this.swapDetailShow = false;
      this.pendingDetailShow = true;
      cheRouterContract.methods
        .swapExactTokensForQKI(amountIn, amountOutMin, path, to, deadline)
        .send({ from: this.account, gas: gasProcessing(gas), gasPrice: web3.utils.numberToHex(this.gasPrice) })
        .on('transactionHash', hash => {
          const message = `${this.fromInput} ${this.fromCur.symbol} ${this.$t('lang.swap1')} ${this.toInput} ${this.toCur.symbol}`;
          const title = this.$t('lang.swap18');
    ...
    async swapTokensForExactQKI(amountOut, amountInMax, path, to, deadline) {
      const cheRouterContract = usecheRouterContract();
      const gas = await cheRouterContract.methods.swapTokensForExactQKI(amountOut, amountInMax, path, to, deadline).estimateGas({ from: this.account });
      this.swapDetailShow = false;
      this.pendingDetailShow = true;
      cheRouterContract.methods
        .swapTokensForExactQKI(amountOut, amountInMax, path, to, deadline)
        .send({ from: this.account, gas: gasProcessing(gas), gasPrice: web3.utils.numberToHex(this.gasPrice) })
        .on('transactionHash', hash => {
          const message = `${this.fromInput} ${this.fromCur.symbol} ${this.$t('lang.swap1')} ${this.toInput} ${this.toCur.symbol}`;
          const title = this.$t('lang.swap18');
    ...
    async swapExactTokensForTokens(amountIn, amountOutMin, path, to, deadline) {
      ...
      const gas = await cheRouterContract.methods
        ...
        .catch(async e => {
          const gas = await cheRouterContract.methods.swapExactTokensForTokensSupportingFeeOnTransferTokens(amountIn, amountOutMin, path, to, deadline).estimateGas({ from: this.account });
          this.swapDetailShow = false;
          this.pendingDetailShow = true;
          cheRouterContract.methods
    ...
    async swapTokensForExactTokens(amountOut, amountInMax, path, to, deadline) {
      const cheRouterContract = usecheRouterContract();
      const gas = await cheRouterContract.methods.swapTokensForExactTokens(amountOut, amountInMax, path, to, deadline).estimateGas({ from: this.account });
      this.swapDetailShow = false;
      this.pendingDetailShow = true;
      cheRouterContract.methods
    ...
    async handleGetAllPair() {
      ...
      const promisePairAddress = allPair.map(item => {
        const cheFactoryContract = getcheFactoryContract();
        const pairAddress = cheFactoryContract.methods.getPair(item.fromCur.address, item.toCur.address).call();
        return pairAddress;
      });
      ...
      let promiseReserve = lpPair.map(item => {
        const chePairContract = getchePairContract(item.lpAddress);
        const reserves = chePairContract.methods.getReserves().call();
        return reserves;
      });
